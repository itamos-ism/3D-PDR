...
#pragma omp parallel for default(none) schedule(dynamic)                                \
    shared(npart, nspec, nreac, nelect, abundance, rate, density, temperature) \
    shared(neq, start_time, end_time, reltol, abstol, mxstep, status)          \
    shared(seconds_in_year) private(i, t0, tout, t, y, data, user_data, cvode_mem, flag, nfails)
#endif
...
//#endif
    ...
    if (check_flag(&retval, "SUNContext_Create", 1))
      status = 1;
    ...
    {
#ifdef LOG_ODES
      if (abundance[n * (*nspec) + i] > 0)
        data[i] = log(abundance[n * (*nspec) + i]);
      else
        data[i] = log(abstol * abstol);
#else
    ...
    if (check_flag((void *)user_data, "malloc", 2))
      status = 1;
...
#pragma omp critical(status)
      ...
      tout = tout * 10;
      if (tout > end_time * seconds_in_year)
        tout = end_time * seconds_in_year;
...
#endif
      if (abundance[n * (*nspec) + i] < abstol)
        abundance[n * (*nspec) + i] = 0;
    }
