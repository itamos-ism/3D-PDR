...
!-----------------------------------------------------------------------
      SUBROUTINE CALC_HEATING(DENSITY,GAS_TEMPERATURE,DUST_TEMPERATURE, &
                 ...
                 & NRGR,NRH2,NRHD,NRCO,NRCI,NRSI,ZETALOCAL) 
...
!     Dust PE heating
      ...
      REAL(KIND=DP) :: HABING_FIELD,DELTA
      REAL(KIND=DP) :: X,XX,XK,XD,GAMMA
...
!     Gas-grain coupling heating/cooling
!      REAL(KIND=DP) :: MECHANICAL_HEATING
...
!-----------------------------------------------------------------------
      ...
      L_TURB=5.0D0
      TURBULENT_HEATING=3.5D-28*((V_TURB/1.0D5)**3)*(1.0D0/L_TURB)*DENSITY!*ABUNDANCE(NH2)
...
!                    & *(DUST_TEMPERATURE-GAS_TEMPERATURE)
      GASGRAIN_HEATING=NGRAIN*CGRAIN*DENSITY*SQRT(8*KB*GAS_TEMPERATURE/(PI*MH)) &
                     & *ACCOMMODATION*(2*KB*DUST_TEMPERATURE-2*KB*GAS_TEMPERATURE) !UCL_PDR
...
!        if (density>1e2) MECHANICAL_HEATING = 1e-24*density
