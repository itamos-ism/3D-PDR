...
from pdrstudio import outputs, params, paths, rttool, runner, ui
...
coolants = rttool.order_coolants(rttool.list_coolants(outdir, prefix))
...
def _hi_h2_bar(m, arr, label, log=True):
    ...
    bar.update_layout(height=110, template="plotly_white",
...
def render_profile(kind: str, value_label: str):
    ...
    with c3:
        show_trans = st.checkbox("HI–H₂ transition line", value=True, key=f"{kind}_t",
                                 disabled=m is None)
    ylog = yscale == "log"
    ...
    fig.update_layout(height=max(360, 340 * nrows), template="plotly_white",
                      paper_bgcolor="white", plot_bgcolor="white", font=dict(color="#262730"),
