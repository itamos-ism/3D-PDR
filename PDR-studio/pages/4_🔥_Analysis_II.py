...
ui.configure_page("Analysis II")
st.title("🔥 Analysis II — heating, cooling, emissivities, level populations")
...
with tab_profiles:
    ...
    fig.update_layout(height=1240, margin=dict(t=120, l=70, r=30, b=55),
    ...
    st.caption("Top row: heating functions. Middle row: cooling functions. Thick gray black-transparent line shows the TOTAL of heating and cooling functions, respectively. "
               "Bottom row: per-coolant `.line.fin` "
               "emissivities and `.spop.fin` level populations."
               + (f"  Dashed line: HI→H₂ transition (Aᵥ = {trans:.4g})." if trans else ""))
