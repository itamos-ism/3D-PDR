...
ui.configure_page("Abundance Profiles")
st.title("📊 Abundance Profiles")
...
with tab_profiles:
    ...
    if x_kind == "n_H":
        ...
        x_trans_2h2 = outputs.value_at_crossing_2h2(X, m.Av, x_HI, x_H2)
    ...
    if trans_2h2: _tcap += f"  Dot-dashed: HI→2H₂ (x(H)=2·x(H₂), 50 % H₂ fraction), Aᵥ={trans_2h2:.4g} mag."
    st.caption(
        f"All panels are log–log versus {x_kind}. Drag to pan, scroll to zoom, double-click to reset, "
        "hover for values, click legend entries to toggle traces."
        + _tcap
