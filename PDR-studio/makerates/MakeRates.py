...
# On systems without Tkinter (e.g. headless clusters) the names imported via
# "from tkinter import *" are absent, so provide a placeholder base class for
# the GUI Application below. It is only ever instantiated when useTkinter=True.
if 'Frame' not in globals():
    Frame = object

# Default options
reactionFile = 'rate22_to_rat05.d'#'medium-Smeudrates.dat'
speciesFile  = 'species_full.d'
outputPrefix = 'test'
sortSpecies = True
logForm = False
fileFormat = 'rate05'
...
# Standard header block written to the start of every generated ODE file in C
# format. Matches the first 28 lines of 3D-PDR's odes_*.c files exactly, so
# MakeRates output can be dropped straight into 3D-PDR without further editing.
ODES_C_HEADER = (
    '/*=======================================================================\n'
    '\n'
...
def read_reaction_file(fileName):
    ...
    if len(data) > 10: fileFormat = 'rate05'
        ...
        try:
            ...
            test = struct.unpack(formatCode, data.encode('latin-1'))
            fileFormat = 'rate99'
        except:
            try:
    ...
    while len(data) != 0 and data != [''] and data != ['\n'] and data != ['\r\n']:
        if fileFormat == 'rate05':
            ...
            data = data[:102]
            data = [field.decode('latin-1') for field in struct.unpack(formatCode, data.encode('latin-1'))]
...
def sort_species(speciesList, nFormation, nDestruction):
    zippedList = sorted(zip(nDestruction, nFormation, speciesList))
    nDestruction, nFormation, speciesList = zip(*zippedList)
    zippedList = sorted(zip(nFormation, nDestruction, speciesList))
...
def find_constituents(speciesList):
    ...
    for species in speciesList:
        ...
        zippedList = sorted(zip(elementMass, elementList, constituents))
    ...
    zippedList = sorted(zip(elementMass, elementList))
...
def write_reactions(fileName, reactants, products, alpha, beta, gamma, labels, fileFormat='rate05'):
    ...
    for n in range(nReactions):
        if fileFormat == 'rate05': output.write(formatCode % (n+1,','.join(reactants[n]),','.join(products[n]),alpha[n],beta[n],gamma[n],','.join(labels[n][0:5])))
